#!/usr/bin/env bash
# D10 position x policy launcher. Usage: run_d10.sh <tag> [extra args]
set -euo pipefail
...
TAG="${1:?usage: run_d10.sh <tag> [extra...]}"
shift || true
RUN=$CLAUDE/outputs/d10-$TAG
SCRATCH=$CLAUDE/.scratch/d10-$TAG
mkdir -p "$RUN"

exec $PY -u "$CLAUDE/codes/d10_position_policy.py" \
